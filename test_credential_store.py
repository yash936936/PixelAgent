...
class TestAvailability:
    def test_unavailable_when_no_real_backend(self):
        """This sandbox's actual keyring backend is keyring.backends.fail.Keyring
        (confirmed: no Secret Service running) -- is_available() must
        report False here for real, unpatched keyring, matching
        at_rest.py's is_available() pattern for DPAPI-on-Linux. No
        monkeypatch needed: this sandbox genuinely has no working backend,
