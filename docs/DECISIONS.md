...
### [2026-08-21] Built all three "scoped future work, not blocking" items: Windows
  Credential Manager migration, automated release rollback, LLM_MODEL SetupWizard field
- **Type:** Feature work closing previously-deferred gaps
- **Scope note:** these three were explicitly deferred in earlier reviews as "future work,
  not blocking" (see this file's Phase 16 Finding 2 entry, `docs/STATUS.md`'s "Next action"
  item 4). Track B's trained risk model, the fourth item in that same list, is NOT included
  here -- it genuinely needs real correction data from live usage that doesn't exist yet
  (downstream of Phase 18's real beta), so there's nothing buildable yet.

- **1. `LLM_MODEL` field in SetupWizard** (`src/gui/setup_wizard_logic.py`,
  `src/gui/widgets/setup_wizard.py`). Added an optional "Gemini model" field to the wizard,
  threaded through `build_env_contents()`/`write_env_file()` with a new `llm_model` param
  that defaults to empty string. Leaving it blank (the default) omits `LLM_MODEL` from the
  written `.env` entirely, so config.py's own default (`gemini-3.5-flash-lite`) applies --
  zero behavior change for anyone who doesn't touch the new field. Logic-layer change fully
  tested (17/17 `tests/test_setup_wizard_logic.py`, 3 new). The widget-layer change (3 lines,
  matching the existing profile/profiles-dir field pattern exactly) could NOT be verified in
  this sandbox -- PySide6 isn't installed here (same pre-existing gap as this project's other
  GUI tests) -- needs a real run of the wizard dialog on the user's machine to confirm it
  renders and saves correctly.

- **2. Windows Credential Manager migration** (Phase 16 Finding 2) -- new module
  `src/security/credential_store.py`, using the `keyring` package (added to
  `requirements.txt`, pinned `keyring==25.5.0`). Deliberately additive, not a replacement:
  `config.py`'s `load()` now checks the credential store ONLY as a fallback after the
  `GEMINI_API_KEY` env var (which already covers `.env` via `load_dotenv()`) -- so nobody
  who hasn't explicitly migrated sees any change at all. `migrate_from_env(env_path)` reads
  the key out of `.env`, stores it via `keyring.set_password()`, and replaces the `.env` line
  with an explanatory comment (not a deletion -- keeps the file's shape clear for anyone
  reading it later). A `python -m src.security.credential_store --migrate` CLI entry point
  is included, matching this project's existing pattern of standalone CLI tools
  (`stress_runner.py`, `beta_report.py`) rather than requiring a code change to invoke.
  Follows `src/security/at_rest.py`'s exact existing graceful-degradation pattern
  (`is_available()` / `_warn_once()`) for the same reason: this Linux dev/test environment
  has no real Windows Credential Manager, confirmed here (`keyring.get_keyring()` returns
  `keyring.backends.fail.Keyring`, i.e. no usable backend) -- exactly the condition
  `is_available()` needs to detect and degrade from. Tested with `keyring`'s
  backend-injection support (a fake in-memory `KeyringBackend` subclass standing in for a
  real OS backend) rather than skipped: 19/19 tests passing
  (`tests/security/test_credential_store.py`), covering get/set/delete round-trips, the
  unavailable-backend fallback, migration (including idempotency -- a second migrate call on
  an already-migrated `.env` correctly does nothing), and `config.load()`'s actual fallback
  behavior (env var still wins if both are somehow set; credential store is used when the env
  var is absent; the original clear `RuntimeError` still fires if neither has a key). Existing
  `config.py` test suite re-run to confirm no regression: 34/34 passing.
  **Still needs, on real Windows hardware:** confirmation that `keyring` actually reaches the
  real Windows Credential Manager via its `pywin32`-backed Windows implementation (this
  sandbox can only prove the fallback path and the logic against a fake backend, not the real
  OS integration) -- run `python -m src.security.credential_store --migrate` for real, then
  confirm the key shows up in Windows' own Credential Manager UI (`rundll32.exe
  keymgr.dll,KRShowKeyMgr` or Control Panel), and that a subsequent `python -m src.main` run
  still works with the `.env` line blanked out.

- **3. Automated release rollback** (Phase 14) -- new workflow
  `.github/workflows/rollback.yml`, `workflow_dispatch`-triggered (deliberately manual-trigger,
  not automatic on any push -- a rollback decision should always be a deliberate human action,
  "without manual intervention" means the mechanics of doing it are automated once decided,
  not that no human is involved in deciding). Takes a `tag` input plus a `confirm` input that
  must match exactly (basic guard against a typo'd tag deleting the wrong release), then uses
  the GitHub CLI (`gh release delete --cleanup-tag`) to remove both the release and its tag in
  one run, falling back to a plain `git push origin :refs/tags/<tag>` if only a bare tag exists
  with no GitHub release object. `release.yml`'s old `rollback-note` job (which only printed
  manual instructions) updated to point at this new workflow instead of describing the gap as
  unfixed. Both YAML files validated with `yaml.safe_load()` -- syntactically correct.
  **Still needs, on the real repo:** an actual end-to-end run against a real throwaway
  tag/release -- like `release.yml` itself, this has never been run for real (no `gh` CLI
  authenticated against a real repo available in this sandbox), so the `gh release delete`
  and tag-deletion steps are standard, well-documented commands but unverified against this
  specific repo's actual permissions/token setup. Confirm `GITHUB_TOKEN`'s default permissions
  are sufficient for `gh release delete` (may need `contents: write`, already granted in the
  job) before relying on this for a real bad release.

- **Honest overall limitation for this entry:** all three items are code-complete and
  unit-tested wherever testable in this sandbox, but all three have a real-environment
  verification step still outstanding (PySide6 GUI render, real Windows Credential Manager,
  real GitHub Actions run) that only the user's actual machine/repo can close. Not marking
  any of these three "done, verified" in `docs/STATUS.md` -- "built and tested here, awaiting
  real-environment confirmation," same honesty standard as Phase 15's own stress-runner entry.

### [2026-08-26] Checkpoint + early-stop for stress_runner.py, after a ~2.9-hour real
  run's summary was lost to an interrupted terminal session; fixed a test-pollution bug
  found while verifying the fix
- **Type:** Bug fix (data-loss prevention) + test-suite correction
- **File(s) affected:** `src/observability/stress_runner.py`, `tests/security/test_credential_store.py`,
  `src/security/credential_store.py`

- **What happened:** a real `--real --hours 4` run was interrupted (terminal/session closed,
  evidenced by a "History restored" marker in the shell) after ~2.9 hours / 17,650 iterations.
  Because `run_stress()` only computes and returns its summary dict after the loop exits
  cleanly, the interruption meant `stress_real_4h_2026-08-21.json` was never written --
  losing the aggregate RSS/thread/error data for the entire run, even though most of it had
  already happened. The per-task `Logger` output under `logs/stress_real/` DID survive
  (confirmed: 416 `.jsonl` files, matching `completed=416` from the last printed progress
  line) since that's written incrementally during the run, not just at the end -- but the
  aggregate numbers (RSS trend, thread count, final error tally) were gone.
- **Secondary, real finding from the same run's data (not a bug, a quota issue):** `completed`
  froze at 416 from roughly iteration 700 through 17,650 while `errors` climbed in lockstep --
  the API quota was fully exhausted partway into hour 1, and every subsequent call failed
  instantly (`RATE_LIMIT_MAX_ATTEMPTS=1` correctly fast-failing rather than sleeping). RSS
  still held in an 81-84MB band across 2.5+ hours of that failure state with `threads=1`
  throughout -- weak but real evidence against a leak, though not the clean signal a
  properly-quota'd run would give.
- **Fix 1 -- periodic checkpointing:** `run_stress()` gained a `checkpoint_path` parameter,
  written every `progress_every` iterations (same cadence as the existing progress-print/
  log-prune) with the current running totals, so an interruption at any point leaves real,
  current numbers on disk instead of losing the whole run. `_main()` defaults
  `--checkpoint` to `<out>.checkpoint.json` automatically whenever `--real` is set (an
  interruption is exactly the case a real multi-hour run needs to be resilient to); fakes-mode
  smoke tests stay checkpoint-free by default since a fakes run finishes in seconds anyway.
  Checkpoint writes are best-effort (`except OSError: pass`) -- a checkpoint failure must
  never crash the actual run.
- **Fix 2 -- early stop on a persistent failure streak:** `run_stress()` also gained
  `stop_after_consecutive_errors`, tracking a `consecutive_errors` counter that resets on any
  completion/limit-stop and triggers a clean early exit (with a clear stderr message, not a
  crash) once it's exceeded. `_main()` defaults this to 100 for `--real` runs. Directly
  targets the quota-exhaustion case above: rather than silently burning through 17,000+
  instant-fail iterations over hours, a real run now stops itself once it's clear nothing
  more will complete, and reports why.
- **Smoke-tested in this sandbox** (fakes mode, 200 iterations): checkpoint file written
  correctly with live totals; full observability + orchestrator-stress suite re-run clean.
  The early-stop path and real quota-exhaustion behavior itself could not be re-exercised
  here (needs a real API key hitting a real 429 streak) -- logically follows the same
  consecutive-error-counting pattern already covered by existing tests' error-path assertions,
  but flagging that the exact trigger condition is unverified against a live API.
- **Separate bug found and fixed while re-verifying the whole suite after this change:**
  `tests/security/test_credential_store.py`'s `_FakeKeyring` test double was subclassing the
  real `keyring.backend.KeyringBackend` with `priority = 1`. Confirmed live: merely DEFINING
  that subclass at module import time silently registers it in `keyring`'s own global
  backend-discovery registry, outranking the real no-op `fail.Keyring` (priority 0) --
  meaning `keyring.get_password()` calls elsewhere in the SAME test session (even in a
  completely different, unrelated test run in isolation) would resolve to a fresh, empty
  instance of this fake instead of the real no-backend condition being tested. This made
  `TestAvailability::test_unavailable_when_no_real_backend` fail with a confusing
  `assert True is False`, exposed only after `credential_store.is_available()` was itself
  corrected (see below) to actually call `get_password()` instead of just `get_keyring()`.
  Fixed by making `_FakeKeyring` a plain duck-typed class with no inheritance from the real
  `KeyringBackend` at all -- it's only ever used via direct `monkeypatch.setattr()` on
  `keyring`'s module-level functions, so real backend registration was never needed and was
  actively harmful. Re-verified: isolated single-test run now passes, full
  `tests/security/test_credential_store.py` (19/19) and full non-GUI suite (446/446) both
  clean.
- **Also fixed in the same pass, found via the same investigation:**
  `credential_store.is_available()` previously called only `keyring.get_keyring()`, which
  returns a working (if no-op) object even with no real backend -- it only raises on actual
  use. Fixed to call `keyring.get_password()` (a real, harmless no-op read against a
  throwaway key name) inside the try/except instead, so unavailability is actually detected
  rather than assumed absent until first real use (which, uncaught, was surfacing as a
  confusing unrelated `NoKeyringError` from deep inside `config.load()`'s fallback path).

### [2026-08-26, continued] Fixed 3 more tests broken by the Credential Manager
  fallback, surfaced only on real Windows hardware with a real backend
- **Type:** Test-suite correction (real environment-dependent gap, same class of bug as the
  test_doctor.py 2026-08-17 entry above)
- **File(s) affected:** `tests/observability/test_stress_runner.py`, `tests/test_config.py`,
  `tests/test_doctor.py`
- **What happened:** after `TestAvailability::test_unavailable_when_no_real_backend` was fixed
  to force the no-backend condition explicitly (see entry above) rather than assume it
  ambiently, running the FULL suite on real Windows hardware surfaced three more failures:
  `test_real_true_without_gemini_api_key_raises_clear_error`, `test_missing_api_key_raises`,
  and `test_check_config_fails_without_api_key` -- all three "DID NOT RAISE"/"assert not True",
  meaning `config.load()` found a usable key when the test expected none. Root cause is the
  same underlying gap in all three: each test simulates "no API key available" by deleting the
  `GEMINI_API_KEY` env var (and, for two of them, stubbing `load_dotenv` per the pre-existing
  2026-08-17 fix). That was a COMPLETE simulation of "no key anywhere" before this session's
  Credential Manager fallback existed -- but `config.load()` now has a third possible source
  (Windows Credential Manager, real and enabled on the user's actual machine), which none of
  these three tests accounted for. This is not a bug in the fallback itself -- it's correctly
  finding a real credential store the user's real machine genuinely has -- it's these tests'
  simulation of "no key" becoming incomplete the moment a second source was added.
- **Fix:** all three tests now also monkeypatch `credential_store.get_api_key` to return
  `None`, making "no key from any source" explicit and machine-independent again, matching the
  same fix pattern as this file's 2026-08-17 `load_dotenv` entry (same class of gap: a test's
  simulated absence needs to cover every real source `config.load()` actually checks, not just
  the first one written).
- **Verified:** all four originally-failing tests (including the `is_available()` one from the
  entry above) now pass together (68/68 in that combined run), and the full non-GUI suite is
  clean at 446/446 -- re-confirmed on this Linux sandbox; still needs a final re-run on the
  user's real Windows machine to close the loop, since that's the only environment that
  actually exercises the real Credential Manager path these tests were missing.
- **Open question for the user, not yet answered:** `check_config()`/`config.load()` finding a
  real key via Credential Manager fallback on a machine that (as far as this session knows)
  never ran `--migrate` is worth understanding -- either a credential was already migrated in
  an earlier session, or something else is populating it. Worth confirming with
